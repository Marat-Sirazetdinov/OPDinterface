...
class App(ctk.CTk):
    ...
    def _register_file_drop_targets(self) -> None:
        ...
        for w in widgets:
            for surf in iter_ctk_drop_surfaces(w):
                # В некоторых окружениях (или без TkinterDnD-обёртки) методы DnD отсутствуют.
    ...
    def setup_data_page(self) -> None:
        ...
        self.btn_data_cancel.pack(anchor="w", pady=(0, 12))
    ...
    def _update_home_plots_after_load(self, preview: Optional[SeismicPreview]) -> None:
        ...
        if preview is None:
            ...
            try:
                ...
                x0 = 0.0
                x1 = float(self.total_traces if self.total_traces > 0 else arr.shape[0])
                axb.imshow(
                    arr.T,
                    aspect="auto",
                    cmap="gray",
                    ...
                    interpolation="bilinear",
                    origin="upper",
                    extent=(x0, x1, float(arr.shape[1]), 0.0),
                )
                axb.set_xlabel("Номер трассы")
                axb.set_ylabel("Время / отсчёт")
                axb.tick_params(labelsize=8)
